...
import { CheckCircle2, BookOpen, Monitor, Wifi, Info, MousePointer, FolderOpen, Video } from "lucide-react";

const PrerequisitosIntroduccion = () => {
  const prerequisites = [
    { text: "Conocimientos básicos de computación", icon: MousePointer },
    { text: "Saber instalar programas", icon: Monitor },
    { text: "Crear y organizar carpetas", icon: FolderOpen },
    { text: "Tomar llamadas por Zoom o similar", icon: Video },
    { text: "Dominio a nivel usuario de tu sistema operativo", icon: MousePointer }
  ];
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
          ...
          <Card className="p-6 bg-gradient-card border-primary/20">
            ...
            <div className="flex items-center gap-2 p-2.5 rounded-lg bg-tech-cyan/10 border border-tech-cyan/20 mb-4">
              ...
              <span className="text-sm text-foreground">Conexión a internet estable (recomendado por cable)</span>
            </div>
          ...
          <Card className="p-6 bg-gradient-card border-primary/20">
            ...
            <div className="space-y-2 mb-4">
              {prerequisites.map((prereq, index) => (
                ...
                >
                  <CheckCircle2 className="w-4 h-4 text-primary flex-shrink-0" />
                  <span className="text-sm text-foreground">{prereq.text}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-start gap-2 p-3 rounded-lg bg-primary/10 border border-primary/20">
              ...
              <p className="text-sm text-foreground">
                <strong>¡No necesitas saber programar!</strong> Este curso es tu punto de partida ideal.
              </p>
            </div>
          </Card>
        </div>
...
export default PrerequisitosIntroduccion;
